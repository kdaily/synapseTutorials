...
def getPermalink(gh, repo, path, ref="master"):
    ## Get the repo
...
def main():
    ...
    parser.add_argument("--cfgfile", type=str, default="~/.pygit")

    args = parser.parse_args()
    
    authtoken = getToken(cfgfile=args.cfgfile)
    g = github.Github(authtoken)

    permalink = getPermalink(gh=g, repo=args.repo, ref=args.ref, path=args.path)
